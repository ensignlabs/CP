...
nf.peoplepicker.sortable = (function () {
    ...
    function _setup() {
        ...
        NWF.FormFiller.Events.RegisterAfterReady(function () {
            ...
            /*    _isMember(NWF$('#' + ctrlID_UserGroup).val());
                if(_isUserCanManageWeb()){
                   console.log("User has full control");
                }
                else{
                   console.log("User has not full control");
                }*/
            ...
            NWF$('.mpp-users-panel.nf-filler-control').each(function () {
                ...
                if (formMode == _formModeType.Display) {
            ...
            if (formMode !== _formModeType.Display) {
                ...
                // Attach click event to 'Add User' button controls
                NWF$('.mpp-add-user').click(function () {
    ...
    function _renderDisplayMode(ctrlPanel) {
        ...
        var userNamesValue = NWF$(ctrlPanel).find('.mpp-usernames textarea').val();
    ...
    function _renderEditMode(ctrlPanel) {
        ...
        // var data = NWF$(ctrlPanel).find('.mpp-data textarea').val();
        ...
        _getSelectedUserInfo(userLoginNames, ppHiddenUsers).then(function () {
            ...
            //     if (jQuery(jQuery(userTag).context).hasClass("ui-state-disabled")) {
            //         jQuery(userTag).next().removeClass("glyphicon-remove-circle")
            //     }
            // });
            ...
            /*  
               jQuery(ctrlSortable).sortable({
                 items: "li:not(.ui-state-disabled)"
              }); 
              
              jQuery( ".mpp-users .mpp-sortable li .ui-state-disabled" ).disableSelection();           
             // disable if tag based on approval status.
              */
        });
    }
    ...
    function _setExistingUserTags(ctrlPanel, existingUsersDetails, userIDs, userApprovals) {
        ...
        // disable if tag based on approval status.
        /*  NWF$(ctrlSortable).find('li .mpp-user-tag').each(function (index, userTag) {
                if(Query(jQuery(userTag).context).hasClass("mpp-inprogress") == true || Query(jQuery(userTag).context).hasClass("mpp-inprogress") == true){
                 // jQuery(".mpp-user-tag .mpp-inprogress").sortable("disable");
                  
                  jQuery(userTag).sortable({ cancel: ".ui-state-disabled" });
                  jQuery(userTag).next().removeClass("glyphicon-remove-circle")
                }
          });*/
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.d.results.length > 0) {
                    ...
                    console.log(_spPageContextInfo.userLoginName + " - " + "user in group");

                } else {

                    console.log(_spPageContextInfo.userLoginName + " - " + "user not in group");
                }
            }
    ...
    function _isUserCanManageWeb() {
        ...
        ctx.executeQueryAsync(
            function () {
                hasPermission = per.get_value() // If this is true, user has permission, if not, no
            },
            function (a, b) {
